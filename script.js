...
const highScoreMsgElement = document.getElementById('highscore');

const effectElement = document.getElementById('fx-layer');

let highScore = Number(localStorage.getItem('highScore')) || 100000;
...
button.addEventListener('click', () => {
  ...
  button.style.backgroundColor = "rgba(169, 169, 169, 1)";

    button.disabled = true; // 버튼 비활성화
    input.disabled = false; // 텍스트 상자 활성화
...
input.addEventListener('input', () => {
  ...
  if (typedValue === currentWord && wordIndex === words.length - 1) { // 마지막 단어까지 정확히 입력했는 지 체크
    ...
    const message = `CONGRATULATIONS! \nYou finished in ${elapsedTime / 1000} seconds.` ;  // 타이핑 완료 메시지
    messageElement.innerText = message; //생성된 메시지 화면에 표시

    button.disabled = false; // start 버튼 다시 활성화
    input.disabled = true; // input 텍스트 상자 비활성화

    if (highScore > elapsedTime || highScore === 0){
      highScore = elapsedTime;
      myStorage.setItem("highScore", String(highScore));
    }
    const high_message = `best score is ${highScore / 1000} seconds.` ;
    highScoreMsgElement.innerText = high_message;
    ...
    const goodMsg = "+ GOOD";
    effectElement.style.color = "rgba(0, 234, 255, 1)";
    effectElement.innerText = goodMsg;
    
  } else {
    typedValueElement.className = 'error'; // 틀리면 error 클래스 추가

    document.body.style.backgroundImage = "url(images/error_background.png)";
    const errorMsg = "+ ERROR";
    effectElement.style.color = "rgba(255, 114, 114, 1)";
    effectElement.innerText = errorMsg;
